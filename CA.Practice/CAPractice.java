...
public class CAPractice {
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i<NUM_MACHINES; i++)  //i = machine number, it goes from 1 - 4
        {
            System.out.println("Enter machine "+(i+1)+" code :");
        ...
        for (int j = 1; j<=maxDays; j++ )     //j is equal to the number of days from the start to the max days
        {
            ...
            if (production<100)
            {
                unitCost = RATE1 * production;
            }
            
            else if (production >=100 && production <=400)
            {
               unitCost = RATE2 * production;
            }
            
            else
            {
                unitCost = RATE3 * production;
            }
